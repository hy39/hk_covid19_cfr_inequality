...
This repository contains the R codes used in the survival
...
to run all the packages/codes mentioned in the top 9 lines of code. You
can run all the codes in the R script to generate the demographic
figures used in the project. If there are such package, make sure to
install these packages first (e.g. install.packages()).


##Survival Analysis

The Project Code_Survival Analysis.R, is divided into
2 parts. -K-M curves, -Cox regression model.

To produce the Kaplan-Meier (K-M) analysis of all covid-19 patients,
first you need to run the following required libraries.
...
Then run the following section to reproduce summary results of the
survival analysis and the K-M plot in total patients.
...
Now to get the K-M survival analysis by age, gender and income region
groups, run the following statements:
...
You can also reproduce the combined graph of above three for the
comparison purpose.
...
Now for the Kaplan-Meier analysis of the symptomatic patients run the
following sections.
...
You can also reproduce the individual survival summary and plots for
different age, gender and income group in symptomatic patients using the section below.
...
Using the following statement you can generate the combined graphs of
the above three groups for the comparative study
...
For the other part of survival analysis, run the
following sections.
